/* Taken from https://github.com/z88dk/z88dk/blob/master/test/suites/stdio/scanf.c */
/* See https://github.com/z88dk/z88dk/blob/master/LICENSE for license */
...
typedef void (*suitetest_f)(void);

/* Fake routines from test.h */
#define Assert(check, msg) Assert_real(check, msg, __FILE__, __LINE__)
void Assert_real(int check, const char *msg, char *file, int line)
{
    if (check)
    {
        printf("Passed check: '%s' at %s line %i\n", msg, file, line);
        suite.npass++;
    }
    else
    {
        printf("FAILED check: '%s' at %s line %i\n", msg, file, line);
        suite.nfail++;
    }
}

void suite_setup(const char *name)
{
...
#define suite_add_test(test) suite_add_test_real(test, STRINGIFY(test))

void suite_add_test_real(suitetest_f func, const char *testname)
{
    printf("Test: %s\n", testname);
    func();
}
...
static nscanf tests[] = {
   ...
   {"0123",   "%o", "%o 0123", 1,83},
   {"123",    "%o", "%o 123", 1, 83},
//   {"%111",   "%B", "%B 111",  1, 7},         // not standard
   {"0123",   "%i", "%i 0123", 1, 83},
   {"0x7fff", "%i", "%i 0x7fff", 1, 32767},     // not standard
//   {"%111",   "%i", "%i 111",  1, 7},
   {"123",    "%i", "%i 111",  1, 123},
   {NULL, NULL, 0 }
};
...
static nslcanf ltests[] = {
   ...
   {"123",    "%lo", "%lo 123", 1, 83},
//   {"%111",   "%lB", "%lB 111",  1, 7}, // not standard
   {"0123",   "%li", "%li 0123", 1, 83},
   {"0x7fff", "%li", "%li 0x7fff", 1, 32767},
//   {"%111",   "%li", "%li 111",  1, 7}, // not standard
   {"123",    "%li", "%li 111",  1, 123},
   {NULL, NULL, 0 }
};
