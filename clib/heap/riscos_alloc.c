...
#define _GetStorageInfo HEAP_TYPEFUNC(RiscosAlloc, GetStorageInfo)
#define _NextHeapElement HEAP_TYPEFUNC(RiscosAlloc, NextHeapElement)
#define _GetEventData HEAP_TYPEFUNC(RiscosAlloc, GetEventData)
#define _GetLastEvent HEAP_TYPEFUNC(RiscosAlloc, GetLastEvent)
#define __heap_checking_on_all_deallocates HEAP_TYPEFUNC(RiscosAlloc, checking_on_all_deallocates)
#define __heap_checking_on_all_allocates HEAP_TYPEFUNC(RiscosAlloc, checking_on_all_allocates)
#define _byte_size HEAP_TYPEFUNC(RiscosAlloc, byte_size)
#define _rma_malloc HEAP_TYPEFUNC(RiscosAlloc, rma_malloc)
#define malloc HEAP_TYPEFUNC(RiscosAlloc, malloc)
#define free HEAP_TYPEFUNC(RiscosAlloc, free)
#define _sys_rma_alloc HEAP_TYPEFUNC(RiscosAlloc, sys_rma_alloc)
#define _sys_alloc HEAP_TYPEFUNC(RiscosAlloc, sys_alloc)
#define __coalesce HEAP_TYPEFUNC(RiscosAlloc, coalesce)
#define _terminate_user_alloc HEAP_TYPEFUNC(RiscosAlloc, terminate_user_alloc)
#define _init_user_alloc HEAP_TYPEFUNC(RiscosAlloc, init_user_alloc)
#define _init_alloc HEAP_TYPEFUNC(RiscosAlloc, init_alloc)
#define realloc HEAP_TYPEFUNC(RiscosAlloc, realloc)
#define calloc HEAP_TYPEFUNC(RiscosAlloc, calloc)
#define aligned_alloc HEAP_TYPEFUNC(RiscosAlloc, aligned_alloc)
...
/*
  ...
  Sourced from: https://gitlab.riscosopen.org/RiscOS/Sources/Lib/RISC_OSLib/-/blob/master/c/alloc
*/
...
#define ACQUIREMUTEX do { (void)heapMutex; } while (0)
#define RELEASEMUTEX do { } while (0)
#define RELEASEANDRETURN(value) do { RELEASEMUTEX; return (value); } while (0);
...
static void *_primitive_alloc(size_t size/*words*/)
{ BlockP block;
  size_t actualSize;
  register int index;
  int fromHighMemory;
  ...
  ACQUIREMUTEX;
#ifdef BLOCKS_GUARDED
  if (check.allocates && check_heap() != OK) RELEASEANDRETURN((void *)(intptr_t)CORRUPT)
#endif
  ...
  FD(size, 10)
  F0(" bytes")
  if (size >= MAXBYTES) RELEASEANDRETURN((void *)(intptr_t)FAILED)
    ...
    if (size <= MAXBINSIZE && lookInBins) { /* get from bin (if not empty) */
      ...
      do {
        ...
        if (block != NULL) { /* got a block */
          if (INVALID(block)) RELEASEANDRETURN((void *)(intptr_t)CORRUPT)
...
#endif
      if (internal_coalesce() != OK) RELEASEANDRETURN((void *)(intptr_t)CORRUPT)
      status |= COALESCED;
    ...
    { BlockP blockCopy;
      ...
      switch (GetMoreOSHeap(size, &blockCopy, &actual)) {
        case OK:
          ...
          if (InsertBlockInOverflowList(block) != OK) RELEASEANDRETURN((void *)(intptr_t)CORRUPT)
          goto get_from_overflow;
        case FAILED:
          block = blockCopy; actualSize = actual;
          if (!enoughMemoryForGC) {
            if (FORCECOALESCE & status) RELEASEANDRETURN((void *)(intptr_t)FAILED)
...
extern void *_rma_malloc(size_t size)
{ void *ptr;
...
extern void *_sys_rma_alloc(size_t n)
{ void *a = _rma_malloc(n);
  if (a == NULL)
...
/*******************************************************************
 Function:      __heap_RiscosAlloc_init
 Description:   Initialise the classic allocator over a fixed heap range
 Parameters:    base = start of heap memory
                size = number of bytes available
 Returns:       none
 ******************************************************************/
void HEAP_TYPEFUNC(RiscosAlloc, init) (void *base, size_t size)
{
  BlockP block;
  size_t span;
  size_t free_size;

  if (size <= OVERHEAD + HOLEOVERHEAD)
  ...
  block = (BlockP)base;
  span = size - HOLEOVERHEAD;
  free_size = span - OVERHEAD;

#ifdef BLOCKS_GUARDED
  block->guard = GUARDCONSTANT;
#endif
  block->size = free_size | FREEBIT;
  block->next = NULL;
  block->previous = NULL;

  endOfLastExtension = ADDBYTES(block, span);
#ifdef BLOCKS_GUARDED
  endOfLastExtension->guard = GUARDCONSTANT;
#endif
  endOfLastExtension->size = 0;

  heapLow = block;
  heapHigh = endOfLastExtension;
  totalFree = free_size;
  totalHeap = PTRDIFF(heapHigh, heapLow);
  userHeap = totalHeap;
  lastFreeBlockOnHeap = block;
  bin[0] = block;
  bin[NBINS+1] = block;
}

/*******************************************************************
 Function:      __heap_RiscosAlloc_alloc
 Description:   Allocate from the classic allocator backend
 Parameters:    size = bytes required
 Returns:       pointer to allocated memory, or NULL
 ******************************************************************/
void * HEAP_TYPEFUNC(RiscosAlloc, alloc) (size_t size)
{
  void *ptr;

  ptr = _primitive_alloc(BYTESTOWORDS(size));
  if ((intptr_t)ptr >= MINHEAPERROR && (intptr_t)ptr < 0)
    return NULL;
...
/*******************************************************************
 Function:      __heap_RiscosAlloc_free
 Description:   Release a block to the classic allocator backend
 Parameters:    block = memory to release
                size = size originally requested
 Returns:       none
 ******************************************************************/
void HEAP_TYPEFUNC(RiscosAlloc, free_backend) (void *block, size_t size)
{
  int rc;
  ...
  rc = _primitive_dealloc((BlockP)block);
  if (rc != OK)
    __heap_fatal("RISC OS allocator free failed");
}

/*******************************************************************
 Function:      __heap_RiscosAlloc_realloc
 Description:   Resize a block using allocate-copy-free semantics
 Parameters:    block = original memory block
                new_size = new size required
                old_size = previous size
 Returns:       pointer to resized block, or NULL
 ******************************************************************/
void * HEAP_TYPEFUNC(RiscosAlloc, realloc_backend) (void *block, size_t new_size, size_t old_size)
{
  void *new_block;
  size_t copy_size;

  if (block == NULL)
    return HEAP_TYPEFUNC(RiscosAlloc, alloc) (new_size);

  if (new_size == 0)
  {
    HEAP_TYPEFUNC(RiscosAlloc, free_backend) (block, old_size);
    return NULL;
  }

  new_block = HEAP_TYPEFUNC(RiscosAlloc, alloc) (new_size);
  if (new_block == NULL)
    return NULL;
...
__heap_implementation_t __heap_riscos_alloc = {
  HEAP_TYPEFUNC(RiscosAlloc, init),
  HEAP_TYPEFUNC(RiscosAlloc, alloc),
  HEAP_TYPEFUNC(RiscosAlloc, free_backend),
  HEAP_TYPEFUNC(RiscosAlloc, realloc_backend),
};
